...
def log_transaction(transaction, initial_balance, final_balance):
    with open('transaction_log.csv', 'a', newline='') as csvfile:
        ...
        # Update the initial and final balance
        transaction['Initial Balance'] = initial_balance
        transaction['Final Balance'] = final_balance
...
def execute_buy_order(symbol, quantity, price, initial_balance, final_balance, sma, ema, rsi, macd):
    try:
        ...
        # Format the quantity and price to the required precision
...
def calculate_profit_loss(initial_balance):
    ...
    for transaction in transactions:
        if transaction['Type'] == 'Buy':
            total_buy_quantity += float(transaction['Quantity'])
            total_buy_cost += float(transaction['Price']) * float(transaction['Quantity'])
        elif transaction['Type'] == 'Sell':
...
def backtest_sma_ema_strategy(symbol, sma_window, ema_window, buy_threshold, sell_threshold, interval, initial_balance):
    try:
        ...
        for i, row in df.iterrows():
            ...
            print("[+] Sleeping...")
            time.sleep(10)  # Sleep for 10 seconds for the next interval
